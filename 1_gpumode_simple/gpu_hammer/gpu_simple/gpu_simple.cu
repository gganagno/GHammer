...

{
	...
	uint64_t * d_first1 = NULL ;
	uint64_t * d_first2 = NULL ;
	uint64_t * d_second1 = NULL;
	uint64_t * d_second2 = NULL;
		    ...
		    j=0;
	      	...
	      	for (uint8_t* second_row_page : pages_per_row[row_index+2]) {
		        ...
		        for (uint8_t* target_page : pages_per_row[row_index+1]) {
		       	...
		       	h_first[0] = (uint64_t *) first_row_page;
		        h_first[1] = (uint64_t *) first_row_page + 0x1000;

		      	h_second[0] = (uint64_t *) second_row_page;
		      	h_second[1] = (uint64_t *) second_row_page + 0x1000;
				...
				*d_sum=0;
		        ...
		        hammer<<<BLOCKS,THREADS>>>(d_first1,d_first2,d_second1,d_second2,d_sum,number_of_reads);
